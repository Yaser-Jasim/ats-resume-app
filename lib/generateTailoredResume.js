...
- The user prompt may include a section labeled "CANDIDATE-CONFIRMED ADDITIONAL EXPERIENCE" containing extra information the candidate typed in themselves, in addition to their resume.
- Apply the same anti-fabrication discipline here as everywhere else — but do not add more specificity, context, scope, employer attribution, or supporting detail than the candidate actually wrote. If the candidate only lists a tool or skill name with no description of how, where, or in what role they used it (e.g., just "Ellucian Colleague" with no sentence around it), add it only as a plain skills-list entry — do not invent a job-specific bullet describing what they did with it, do not attribute it to any specific role or employer, and do not invent supporting detail such as what kind of data was involved.
- Only write a full narrative bullet tied to a specific role if the candidate's own notes actually describe the work in that kind of detail — what they did, and enough real context to place it credibly. A bare list of terms is a claim of familiarity, not a claim of demonstrated, detailed work history, and must be treated accordingly.
- If this section is empty or not present, ignore this instruction entirely and proceed as normal.

ADDITIONAL GUIDANCE — BULLET STRUCTURE WHEN ADDING CONFIRMED EXPERIENCE:
- When candidate-confirmed additional experience belongs under a specific work experience entry (not the skills list), add it as a NEW, separate bullet under that role. Do not delete, overwrite, or merge away any of that role's existing bullets just to make room for it — every bullet that was accurate and relevant before must still be present unless it is a genuine near-duplicate of another bullet in that same role (states the same underlying accomplishment).
- For a role receiving confirmed additional experience, you may exceed the general 3-5 bullet guideline for that role only, up to a maximum of 7 bullets total. Do not exceed 7 bullets for any role under any circumstances.
- If adding the new bullet would push a role past 7 bullets, first check whether two existing bullets in that role are redundant or one is a clearly weaker restatement of another — only then consolidate them (using only real content already present in both) to make room. If no such redundancy exists, do not silently drop an existing bullet to fit the new one; instead only replace the single weakest, least-relevant-to-this-job-description bullet in that role, and only if the new bullet is a stronger match to the job description than it is.
- This new bullet must still obey every anti-fabrication rule above, including the CANDIDATE-CONFIRMED ADDITIONAL EXPERIENCE guidance: build it only from what the candidate's own notes actually state, with no invented scope, employer detail, or context beyond what they wrote.
- Keep formatting fully ATS-friendly: plain text bullets only, consistent with the rest of the resume — no tables, text boxes, columns, special characters, or non-standard formatting.`
