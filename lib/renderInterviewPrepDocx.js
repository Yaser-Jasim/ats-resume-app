...
export async function renderInterviewPrepDocx({ candidateName, positionTitle, orgName, prep }) {
  ...
  if (prep.opening_impression?.length) {
    children.push(heading('Starting Strong: Your First Two Minutes'))
    prep.opening_impression.forEach(t => children.push(body(`•  ${t}`)))
  }

  if (prep.demeanor_tips?.length) {
    children.push(heading('Reading the Room: Tone Throughout the Interview'))
    prep.demeanor_tips.forEach(t => children.push(body(`•  ${t}`)))
  }

  children.push(heading('Likely Questions & How to Answer Them'))
  ...
  if (prep.closing_impression?.length) {
    children.push(heading('Ending Strong: Your Last Few Minutes'))
    prep.closing_impression.forEach(t => children.push(body(`•  ${t}`)))
  }

  children.push(heading('Staying Calm'))
  prep.stress_tips?.forEach(t => children.push(body(`•  ${t}`)))

  if (prep.after_interview?.length) {
    children.push(heading('After You Leave'))
    prep.after_interview.forEach(t => children.push(body(`•  ${t}`)))
  }
